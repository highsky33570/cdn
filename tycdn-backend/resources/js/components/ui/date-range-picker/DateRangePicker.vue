...
function inRange(d: Date, s: Date | null, e: Date | null): boolean {
    ...
    return (
        t > Math.min(s.getTime(), e.getTime()) &&
...
function buildMonth(year: number, month: number): Cell[] {
    ...
    const effEnd =
        hoverDate.value && startDate.value && !endDate.value
            ? hoverDate.value
            : endDate.value;
    ...
    for (let d = 1; d <= last.getDate(); d++) {
        ...
        cells.push({
            ...
            isEnd:
                sameDay(cur, endDate.value) ||
                (!endDate.value && sameDay(cur, hoverDate.value)),
...
const rightCells = computed(() =>
    buildMonth(rightYear.value, rightMonth.value),
);
...
function setPreset(preset: 'today' | 'yesterday' | '7d' | '30d'): void {
    ...
    if (preset === 'today') {
        startDate.value = new Date(t);
        endDate.value = new Date(t);
    } else if (preset === 'yesterday') {
        const y = new Date(t);
        y.setDate(y.getDate() - 1);
        startDate.value = y;
        endDate.value = new Date(y);
    } else if (preset === '7d') {
        const s = new Date(t);
...
function syncSelection(): void {
    startDate.value = parseDate(props.start);
    ...
    if (startDate.value) {
        ...
        const next = new Date(leftYear.value, leftMonth.value + 1, 1);
        rightYear.value = next.getFullYear();
        rightMonth.value = next.getMonth();
    }
}
watch(() => [props.start, props.end], syncSelection, { immediate: true });
watch(open, (value) => {
    if (value) {
        syncSelection();
    }
...
function cellClass(cell: Cell): string {
    const base =
        'flex h-7 w-7 mx-auto items-center justify-center rounded-md text-xs select-none transition-colors';
    ...
    if (cell.inRange) {
        return `${base} cursor-pointer bg-accent text-accent-foreground`;
    }

    if (cell.isToday) {
        return `${base} cursor-pointer font-medium hover:bg-accent`;
    }
            ...
            <div @mouseleave="hoverDate = null">
                ...
                <div class="flex gap-1.5 border-b px-3 py-2">
                    <button
                        ...
                        :key="p.key"
                        type="button"
                        class="rounded-full border px-3 py-0.5 text-xs text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground"
                        @click="
                            setPreset(
                                p.key as 'today' | 'yesterday' | '7d' | '30d',
                            )
                        "
                    >
                        {{ p.label }}
                ...
                <!-- 双月日历 -->
                <div
                    class="flex max-w-[calc(100vw-2rem)] flex-wrap gap-0 px-3 py-3"
                >
                    <!-- 左月 -->
                    <div class="w-52">
                        ...
                        <div class="mb-1 grid grid-cols-7">
                            <div
                                ...
                                :key="w"
                                class="py-0.5 text-center text-[10px] text-muted-foreground"
                            >
                                {{ w }}
                            </div>
                        </div>
                    ...
                    <!-- 分隔线 -->
                    <div class="mx-3 w-px self-stretch bg-border" />
